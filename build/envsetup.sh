...
- githubremote:    Add git remote for Helios-Project Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Helios-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        adb wait-for-online
        echo "Found device"
        if (adb shell getprop org.helios.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/Helios-Project/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-online
    if (adb shell getprop org.helios.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.helios.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
